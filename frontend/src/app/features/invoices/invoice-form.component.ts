...
const DEFAULT_PAYMENT_TERMS = 'Paiement à 30 jours, par virement bancaire.';
  ...
  template: `
    <div style="padding:24px; max-width:1100px;">
      ...
      <h1 style="margin:0 0 16px;">
        {{ editingId() ? 'Edit ' + (invoiceNumber() || 'invoice') : 'New invoice' }}
      </h1>
      ...
      <form *ngIf="!loadingInvoice() && !loadError()" [formGroup]="form" (ngSubmit)="onSubmit()">
        ...
        <div style="display:flex; gap:8px; justify-content:flex-end; margin-top:16px;">
          <a mat-button routerLink="/invoices" [class.mat-button-disabled]="loading()">Cancel</a>
    ...
    this.invoices.get(id).subscribe({
      next: inv => {
        if (inv.status !== 'DRAFT') {
          ...
          this.snack.open('Only DRAFT invoices can be edited.', 'Dismiss', { duration: 3000 });
      ...
      error: err => {
        this.loadingInvoice.set(false);
        this.loadError.set(extractErrorDetail(err, 'Invoice not found.'));
      }
    });
  }
    ...
    request$.subscribe({
      ...
      error: err => {
        ...
        if (fieldErrors) {
          ...
          this.error.set(extractErrorDetail(err, 'Could not save invoice.'));
        }
      }
    });
  }
